...
def w_score(form):
    pts = {"W": 3, "D": 1, "L": 0}
    weights = [1, 1, 1, 1.5, 1.5]
    s = 0
    for i in range(5):
        s += pts.get(form[i], 0) * weights[i]
    return s
...
for m in maclar[:60]:
    ...
    cards += '<div class="row"><span>Eksik:' + str(eh) + '</span><span>xG ' + str(xh) + '-' + str(xa) + ' T=' + str(xsum) + '</span><span>' + str(ea) + ':Eksik</span></div>'
...
html = ""
html += "<!DOCTYPE html><html lang='tr'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1'><title>24 Saat</title>"
html += "<style>body{margin:0;background:#0a0c10;color:#fff;font-family:system-ui;padding:0 0 40px}.top{background:#ffcc00;color:#000;text-align:center;padding:8px;font-size:12px;font-weight:800}.grid{max-width:1200px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:14px;padding:0 12px}.card{background:#151922;border:1px solid #253047;border-radius:18px;padding:14px}.lig{font-size:11px;opacity:.5}.mac{font-size:20px;font-weight:900;margin:10px 0;display:flex;justify-content:space-between}.vs{opacity:.3;font-size:14px}.row{display:flex;justify-content:space-between;font-size:12px;background:#0f131b;padding:8px 10px;border-radius:10px;margin-top:6px}.bar{height:7px;display:flex;border-radius:10px;overflow:hidden;margin-top:10px;background:#0f131b}span.W{background:#1f8a4c;color:#fff;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;border-radius:5px;margin-right:3px}span.D{background:#444;color:#fff;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;border-radius:5px;margin-right:3px}span.L{background:#7a2a2a;color:#fff;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;border-radius:5px;margin-right:3px}</style>"
html += "</head><body><div class='top'>Egitim / eglence amacli istatistiksel analizdir, bahis tavsiyesi degildir.</div>"
html += "<h1 style='text-align:center'>Onumuzdeki 24 Saat - " + str(len(maclar)) + " mac</h1>"
html += "<div class='grid'>" + cards + "</div></body></html>"

open("index.html", "w", encoding="utf-8").write(html)
print("OK " + str(len(maclar)))
